...
TICK_STATIC_TEST_CASE()
{
    template<class T, TICK_REQUIRES(tick_is_integral<T>())>
    static std::true_type check_requires(T);
...
TICK_STATIC_TEST_CASE()
{
    template<class T, class... Ts, TICK_REQUIRES(tick_is_integral<T>())>
    static std::true_type check_vardiac_requires(T, Ts...);
...
(std::true_type) check_function_requires_c(T);

template<class T> 
TICK_FUNCTION_REQUIRES(!std::is_integral<T>::value)
(std::false_type) check_function_requires_c(T);

TICK_STATIC_TEST_CASE()
{
    STATIC_ASSERT_SAME(decltype(check_function_requires_c(1)), std::true_type);
    STATIC_ASSERT_SAME(decltype(check_function_requires_c(not_int())), std::false_type);
};

template<class T> 
TICK_FUNCTION_REQUIRES(tick_is_integral<T>())
...
TICK_STATIC_TEST_CASE()
{
    ...
    template<class T>
    struct check_class_requires<T, TICK_CLASS_REQUIRES(tick_is_integral<T>())>
    {
        static std::true_type call();
    };
...
TICK_STATIC_TEST_CASE()
{
    ...
    struct check_member_requires
    {
        ...
        TICK_MEMBER_REQUIRES(tick_is_integral<T>())
        std::true_type foo()
        {
            x += 1;
            return std::true_type();
...
template<class T>
std::true_type check_param_requires(T, TICK_PARAM_REQUIRES(tick_is_integral<T>()));

template<class T>
std::false_type check_param_requires(T, TICK_PARAM_REQUIRES(!tick_is_integral<T>()));
